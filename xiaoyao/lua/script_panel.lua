...
local mainTask = require "task.maintask"
...
script_panel.init = function()
	...
	script_panel.mainTaskBtn = lua_button1.new()
	script_panel.mainTaskBtn:create(script_panel.rootwin, "主线", 150, 120)
	script_panel.mainTaskBtn:sub_click_event(script_panel.on_click_main_task)	
end
...
script_panel.checkReStart = function()
	...
	if script_panel.lastTask then
		if false == ClientControl.is_moving() then
			...
			if lua_app.RestartIndex >= 5 then
				lua_app.RestartIndex = 0
...
script_panel.click_btn_effect = function(btn, task)
	if script_panel.lastClickBtn then
		local x = script_panel.lastClickBtn.x
		script_panel.lastClickBtn:set_x(x-30)
	end
	btn:set_x(btn.x + 30)
...
script_panel.on_click_main_task = function(click_type)
	script_panel.click_btn_effect(script_panel.mainTaskBtn, mainTask.new())

	lua_app.task_type = "主线任务"
	lua_app.print(7, "lua_app.task_type = ", lua_app.task_type)
end
