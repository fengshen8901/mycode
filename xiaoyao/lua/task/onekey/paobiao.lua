...
function pao_biao:onPatchEndLingQu()
    ...
    local entityId = common.getNpcEntiyIdByName("张镖头")
    if entityId then
        lua_app.print(7, "pao_biao:logic 点击 张镖头 ")
        ClientControl.open_mutual(entityId)
        eventMgr.subEvent(eventMgr.event.open_mutual, self, "onOpenLingQuMutual")
    else
        lua_app.print(7, "pao_biao:logic 找不到 张镖头 ")
    end
end
